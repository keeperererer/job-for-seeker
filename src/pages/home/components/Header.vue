...
<div class="header">
	<ul>
		...
		<li v-for="nav in navList" :key="nav.id"><router-link :to="nav.list" class="li">{{nav.title}}</router-link></li>
	 	...
	 	<div class="modal-dialog" style="width:30%">
        	<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<input type="button" value="登录"  data-dismiss="modal" class="btn btn-primary"  @click="submitForm('ruleForm')">
				</div>
			</div>
		</div>
	</form>
 	...
 	<form style="margin-top:60px;">
	 	<div class="modal-dialog" style="width:30%">
        	<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-group" v-for="form2 in formList2" :key="form2.id">
...
export default {
	...
	methods:{
		...
		judge(){
			...
			let _this = this;
			$.ajax({
	          url: API_PROXY + 'http://47.100.121.23:8080/job/user/register',
	          type: 'post',
	          // data对象中的属性名要和服务端控制器的参数名一致 login(name, password)
	          data: {
	            'useraccount': this.formList2[0].value,
							'password': this.formList2[3].value,
							're_password': this.formList2[4].value
	          },
	          dataType: 'JSONP',
		...
		submitForm(formName) {
			...
			let _this = this;
			$.ajax({
	          url: API_PROXY + 'http://47.100.121.23:8080/job/user/login',
	          type: 'post',
	          // data对象中的属性名要和服务端控制器的参数名一致 login(name, password)
	          data: {
	            'useraccount': _this.ruleForm.username,
	          ...
	          success: function (response) {
	          	...
	          	console.log(response.ret)
		 					if(response.ret) {
		 						...
		 						_this.loginedShow = true;
		 						// console.log(_this.loginShow)
		 						// console.log(_this.loginedShow)
		 						}

		 					if(!response.ret){
		 						console.log("登录失败")
		 						_this.loginShow = true
		 						_this.loginedShow = false
		 						alert('登录失败,如果没有注册请先注册，再检查账号密码！')
		 					}
	          },
	          error: function (e) {
	            console.log(e)
